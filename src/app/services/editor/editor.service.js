...
function YAMLgenerator(title, tags = [], images = []) {
	...
	return YAML.stringify({ Title: title, Contents: contents, Metadata: metadata });
}
...
const EditorService = {
	...
	storeImages: async (blogID, images) => {
		const metadataResponse = await S3Service.getObject('meta/' + blogID + '.yaml');
		...
		const metadataUploaded = await S3Service.upload('meta/' + blogID + '.yaml', YAML.stringify(metadata));
